...
- No SNS subscriptions are made by the deployment process. The topic is created, and several other services are configured to publish notifications to the topic, but the topic itself is not automatically configured to fan out any notifications. Here's why:
  ...
  - We likely only want to subscribe to notifications for higher/long running environments like main, staging, and production.
...
- The SNS topic ARN is injected via a serverless compose parameter (see [serverless-compose.yml](../blob/main/serverless-compose.yml)) so other services can be configured to publish to it.